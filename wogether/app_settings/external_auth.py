'''Configure auth-related libraries.'''
# coding: utf-8
...
# Specifies the login method to use – whether the user logs in by entering their username, e-mail address, or either
# one of both. Setting this to “email” requires ACCOUNT_EMAIL_REQUIRED=True
ACCOUNT_AUTHENTICATION_METHOD = "email"

# ACCOUNT_CONFIRM_EMAIL_ON_GET (=False)
# Determines whether or not an e-mail address is automatically confirmed by a GET request. GET is not designed to
# modify the server state, though it is commonly used for email confirmation. To avoid requiring user interaction,
...
# In order to verify an email address a key is mailed identifying the email address to be verified. In previous
# versions, a record was stored in the database for each ongoing email confirmation, keeping track of these keys.
# Current versions use HMAC based keys that do not require server side
# state.
...
# Determines the e-mail verification method during signup – choose one of “mandatory”, “optional”, or “none”. When set
...
# specified ACCOUNT_LOGIN_ATTEMPTS_TIMEOUT seconds. Set to None to disable this functionality. Important: while this
# protects the allauth login view, it does not protect Django’s admin login from being brute forced.
ACCOUNT_LOGIN_ATTEMPTS_LIMIT = 5

# ACCOUNT_LOGIN_ATTEMPTS_TIMEOUT (=300)
# Time period, in seconds, from last unsuccessful login attempt, during which the user is prohibited from trying to
# log in.
...
#
# By changing this setting to True, users will automatically be logged in once they confirm their email address. Note
#  however that this only works when confirming the email address immediately after signing up, assuming users didn’t
...
# Determines whether or not the user is automatically logged out after changing or setting their password. See
# documentation for Django’s session invalidation on password change. (Django 1.7+)

# ACCOUNT_LOGIN_ON_PASSWORD_RESET (=False)
# By changing this setting to True, users will automatically be logged in once they have reset their password. By
# default they are redirected to the password reset done page.
...
# Controls the life time of the session. Set to None to ask the user (“Remember me?”), False to not remember, and True
#  to always remember.
...
# for additional input (e.g. newsletter signup, birth date). This class should implement a def signup(self, request,
# user) method, where user represents the newly signed up user.
...
ACCOUNT_USER_DISPLAY = 'wog_user.models.get_user_display_name'
...
# An integer specifying the minimum allowed length of a username. Deprecated – use Django’s AUTH_PASSWORD_VALIDATORS
# instead.

# ACCOUNT_USERNAME_REQUIRED (=True)
# The user is required to enter a username when signing up. Note that the user will be asked to do so even if
# ACCOUNT_AUTHENTICATION_METHOD is set to email. Set to False when you do not wish to prompt the user to enter a
# username.
